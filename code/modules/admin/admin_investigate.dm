	...
	switch(subject)
		if("singulo", "telesci", "atmos", "chemistry")			//general one-round-only stuff
			...
			if(!F)
				...
				return
			src << browse(HTML_SKELETON_BODY(F),"window=investigate[subject];size=800x300")

		if("hrefs")				//persistant logs and stuff
			if(config && config.log_hrefs)
				if(href_logfile)
					src << browse(HTML_SKELETON_BODY(href_logfile),"window=investigate[subject];size=800x300")
				else
